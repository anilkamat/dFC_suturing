...
from plot_mesh_grid_08192022 import plot_3d_mesh, analysis_tuned_model
import warnings
warnings.filterwarnings('ignore')
s_time = time.time()
config={
    #'PATH' :r'C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\EEG_fNIRS_paper_Brain_informatics\EEG_codes_results\SVM_results_05242023\T3P4',  # SVM results storing directory
    ...
    # 'phase':['P1','P2','P3','P4'],
    'phase':['ST1','ST2','ST3','ST4','ST5','ST6','ST7','ST8','ST9','ST10','ST11','ST12','ST13'], # for 13 subtasks of EEG ]
    'Trials' : ['T1'],#,'T2','T3'], #,'Tr_3']
    'model_para_grid':{'c' :np.arange(0.001,20,0.5),  # 'model_para_grid':{'c' :np.arange(0.1,10,0.5),# if change value, change in the grid search fxn as well
    'gamma':np.arange(0.001,20,0.5),   #'gamma':np.arange(0.1,10,0.5), 
    'kernel_' : ['rbf']}
}
...
remove_columns = ['LPFC-->SMA','RPFC-->SMA','LPMC-->SMA','RPMC-->SMA','SMA-->LPFC','SMA-->RPFC','SMA-->LPMC','SMA-->RPMC'] # remove columns which don't have enough number of ICs (ex: SMA)
for T in config['Trials']:
    for P in config['phase']:
        # file_ = 'SA_normal_'+str(T)+str(Tr)+'.csv'
        ...
        filename = os.path.join(r'C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\EEG_fNIRS_paper_Brain_informatics\EEG_codes_results\GC_regional_Jun10\Connectivities',csvfilename)
        savePath_results = os.path.join(r'C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\EEG_fNIRS_paper_Brain_informatics\EEG_codes_results\GC_regional_Jun10\SVM_results_05242023',T+P)
        #filename = r'C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\EEG_fNIRS_paper_Brain_informatics\EEG_codes_results\GC_regional_Jun10\Connectivities\Phases\EEG_GC_Exp_Nov_Phases_T3P4.csv'
        df = pd.read_csv(filename, header=0, index_col=False, sep=',')
        df.drop(remove_columns, axis=1, inplace=True)  # for EEG drop the connetivity with region SMA as there are no ICs in SMA regions. 
        df = df[(df!=0).all(1)]
        connections = df.columns[0:12]  # for EEG there are only 12 columns i.e. df.columns[0:12]
        ...
        para_model_allConnectivity, best_model_allConnectivity, para_model_allConnectivity_mcc, para_model_allConnectivity_bas \
            = grid_search(df,config_trial,config,set_number,savePath_results, Type='all_connectivity')  # grid search for best parameters 
        # pickle.dump(para_model_allConnectivity, open('para_model_allC'+'.sav', 'wb'))
        # with open(os.path.join(savePath_results,'best_model_allC.sav'), 'wb') as f:
        #     pickle.dump(best_model_allConnectivity, f)
        ...
        analysis_tuned_model(df,set_number,connections, para_model_allConnectivity[str(ModelName)], str(ModelName),m,config,best_model_allConnectivity,savePath_results,connections, type='acc')
        ...
        #fname_mtx = 'Metrics'+T+P+'.xlsx'  # storcsbest model metrices.
        ...
        set_number = 'dummy' # this set is just used as buffer/ dummy set_number
        for connection in connections:  # test one single connectivity at a time and select top performer for futher forming sets for refin_connectivity
            ...
            para_model_,best_model_, para_model_mcc, para_model_bas \
                =grid_search(data,config_trial,config,set_number,savePath_results,Type='add_connectivity')  # grid search for best parameters 
            dict[connection] = best_model_[ModelName+'test_acc_CV']
            plot_3d_mesh( para_model_[str(ModelName)], str(ModelName),m,config,best_model_, savePath_results,  connection, type = 'acc', conn_num = i)
            plot_3d_mesh( para_model_mcc[str(ModelName)], str(ModelName),m,config,best_model_, savePath_results,connection, type = 'mcc', conn_num = i)
            plot_3d_mesh( para_model_bas[str(ModelName)], str(ModelName),m,config,best_model_, savePath_results, connection, type = 'bas', conn_num = i)
            if best_model_[ModelName+'gamma'] != 0.0: 
                analysis_tuned_model(data,set_number,connection, para_model_[str(ModelName)], str(ModelName),m,config,best_model_,savePath_results,i, type='acc')
            i+=1
        sort_orders = sorted(dict.items(), key=lambda x: x[1], reverse=True)
        print('sort_orders, dict.items(): ', sort_orders, dict.items())
        ...
        for i in range(1): # for loop to write the metrices of the single_connectivity with maximum accuracy
            ...
            plot_3d_mesh( para_model_[str(ModelName)], str(ModelName),m,config,best_model_,savePath_results, top_single_conn, type = 'acc')
            plot_3d_mesh( para_model_mcc[str(ModelName)], str(ModelName),m,config,best_model_,savePath_results, top_single_conn, type = 'mcc')
            plot_3d_mesh( para_model_bas[str(ModelName)], str(ModelName),m,config,best_model_,savePath_results, top_single_conn, type = 'bas')
            analysis_tuned_model(data,set_number,top_single_conn, para_model_[str(ModelName)], str(ModelName),m,config,best_model_,savePath_results,i, type='acc')

        # with open(os.path.join(savePath_results,'best_model_singleC.sav'), 'wb') as f:
        ...
        def refine_connectivity(set1,set3,df,set1_top_acc,set_number): # refines the discering connectivities.
            ...
            for connection in set3:
                ...
                if acc_set1 > set1_top_acc:  # if acc increased
                    ...
                    set3 = set3.drop(connection)   # dorp the connection if it didn't improve the accuracy
                    ...
                    break
            print(f'set1 (adding connectivities): {set1}, set3: {set3}')
        ...
        dict2 = selected_conn
        while(flag):  # loop for other(except baseline and single connectivity) sets of connectivities
            # set2 = connections
            if len(set3) >=1:
