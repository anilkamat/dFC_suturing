...
from chEEG_grid_search_parameters_subtask_LOOCV_06242023 import grid_search
from chEEG_plot_mesh_grid_LOOCV_06242023 import plot_3d_mesh, analysis_tuned_model
import warnings
...
config={
  'trials': ['T1','T2','T3'], 
  'phases' : ['P1'],#'P3','P4'],
    'model_para_grid':{'c' :np.arange(0.001,20,10),  # 'model_para_grid':{'c' :np.arange(0.001,10,0.5),# if change value, change in the grid search fxn as well
    'gamma':np.arange(0.001,20,10),   #'gamma':np.arange(0.001,10,0.5), 
    'kernel_' : ['rbf']}
}
filenames = []
...
for T in config['trials']:
    for P in config['phases']:
        ...
        filename = os.path.join(r'C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\EEG_fNIRS_paper_Brain_informatics\channelEEG_codes_results\Results\Connectivities',csvfilename)
        savePath_results =  os.path.join(r'C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\EEG_fNIRS_paper_Brain_informatics\channelEEG_codes_results\Results\SVM_06182023',T+P)        
